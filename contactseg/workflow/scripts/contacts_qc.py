...
def clean_svgs(bg1_svgs_strs, bg2_svgs_strs, ref=0):
    ...
    for f in svgs:
        ...
        if vb is None:
            raise ValueError(
                "SVG missing viewBox attribute. Matplotlib SVGs must have it."
            )
...
def extract_trajectory_slice(nifti_img, entry, exit, width=40, num_points=256):
    ...
    # reference axis
    ref_axis = (
        np.array([1, 0, 0]) if abs(np.dot(u, [1, 0, 0])) < 0.95 else np.array([0, 0, 1])
    )
...
def render_oblique_slice_to_svg(img, entry_world, exit_world, points, **args):
    ...
    slice_img, s_vals, t_vals, u, v, w = extract_trajectory_slice(
        img, entry_world, exit_world
    )
    ...
    plt.scatter(
        entry_st[1], entry_st[0] + offset, color="green", label="Entry Point", s=100
    )
    plt.scatter(
        exit_st[1], exit_st[0] + offset, color="red", label="Target Point", s=100
    )
    ...
    for pt, label in points:
        ...
        if s is not None and t is not None:
            ...
            plt.text(
                t + 1, s + 1, str(label), color="white", fontsize=10, weight="bold"
            )
    ...
    plt.savefig(
        svg_buffer, format="svg", bbox_inches="tight", pad_inches=0, facecolor="black"
    )
...
def output_html_file(
    ...
    for label, points in contacts.items():
        ...
        bg_x_ct_contacts_svgs = render_plane_slice_to_svg(
            ct_img, middle_point, points, axis="x", **matplotlib_refs_ct
        )
        bg_y_ct_contacts_svgs = render_plane_slice_to_svg(
            ct_img, middle_point, points, axis="y", **matplotlib_refs_ct
        )
        bg_z_ct_contacts_svgs = render_plane_slice_to_svg(
            ct_img, middle_point, points, axis="z", **matplotlib_refs_ct
        )

        # For T1w + contacts overlays (background group 2)
        bg_x_t1w_contacts_svgs = render_plane_slice_to_svg(
            t1w_img, middle_point, points, axis="x", **matplotlib_refs_t1w
        )
        bg_y_t1w_contacts_svgs = render_plane_slice_to_svg(
            t1w_img, middle_point, points, axis="y", **matplotlib_refs_t1w
        )
        bg_z_t1w_contacts_svgs = render_plane_slice_to_svg(
            t1w_img, middle_point, points, axis="z", **matplotlib_refs_t1w
        )

        # For trajectory slice
        svg_oblique_ct = render_oblique_slice_to_svg(
            ct_img, entry_world, exit_world, points, **matplotlib_refs_ct
        )
        svg_oblique_t1w = render_oblique_slice_to_svg(
            t1w_img, entry_world, exit_world, points, **matplotlib_refs_t1w
        )

        final_svg_x = "\n".join(
            clean_svgs([bg_x_ct_contacts_svgs], [bg_x_t1w_contacts_svgs])
        )
        final_svg_y = "\n".join(
            clean_svgs([bg_y_ct_contacts_svgs], [bg_y_t1w_contacts_svgs])
        )
        final_svg_z = "\n".join(
            clean_svgs([bg_z_ct_contacts_svgs], [bg_z_t1w_contacts_svgs])
        )
